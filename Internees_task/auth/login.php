...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $connection->prepare("SELECT * FROM users WHERE username = ? AND is_active = 1");
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        // Update last login
        $update_stmt = $connection->prepare("UPDATE users SET last_login = NOW() WHERE user_id = ?");
        ...
        // unset($_SESSION['redirect_url']); // Clear after use
            ...
            $redirect_url = $_SESSION['redirect_url'] ?? '/NIRDAKMS/DiscussionForum/manage_forums.php';
...
<head>
    ...
    <title>Login | NIRDA Knowledge Management System</title>
    <style>
        body {
